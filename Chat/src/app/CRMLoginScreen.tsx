...
import { ArrowEnter20Filled, Chat20Filled } from '@fluentui/react-icons';
...
const LOGIN_TITLE = 'Agent login to CRM';
 ...
 * @param props
 */
export default (props: CRMLoginScreenProps): JSX.Element => {
  ...
  const { joinChatHandler, setToken, setUserId, setDisplayName, setThreadId, setEndpointUrl, setWorkerId } = props;
  ...
  const setupAndJoinChatThreadWithNewUser = useCallback(() => {
    const internalSetupAndJoinChatThread = async (): Promise<void> => {
      ...
      // register user as worker for job router
      const workerId = token.identity.split(':')[2];
      const result = await registerWorker(workerId, name);
      if (result) {
  ...
  const displayJoinChatArea = (): JSX.Element => {
    return (
      <Stack horizontalAlign="center" verticalAlign="center" tokens={responsiveLayoutStackTokens} className={mainContainerStyle}>
        <Text className={headerStyle}>
          {LOGIN_TITLE}
        </Text>
        <Stack
          horizontal
        ...
        >
          ...
          <Stack className={rightInputContainerStyle} tokens={rightInputContainerStackTokens}>
            ...
            <PrimaryButton
              ...
              onRenderIcon={() => <ArrowEnter20Filled className={chatIconStyle} />}
            />
          </Stack>
        </Stack>
      </Stack>
