 ...
 */
export const acceptJobOffer = async (workerId: string,offerId: string): Promise<boolean> => {
  try {
    const response = await fetch('/acceptJobOffer', {
      method: 'POST',
      headers: {
    ...
    console.error('Failed at acceptJobOffer, Error: ', error);
  }
  return false;
};
